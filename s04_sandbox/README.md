...
在 dispatch 外面再包一层**沙箱**：拦截**每一个**工具调用（工作区里的写入也一样），判断它想碰哪，再对照 `sandbox_mode` 决定放行还是拒绝。被拒绝的调用不会崩溃——harness 把一条错误 item 喂回模型，模型读到「越界被拒」后换一条路继续。

沙箱不是「工作区外才启用」，也不是先改一份副本再同步回来。命令仍然写**真实文件**；沙箱是套在这条进程上的笼子。`workspace-write` 只是把笼子的可写根设成启动目录：屋里的改动当场落盘，屋外和（真实 Codex 里常常还有）网络仍然出不去。只有 `danger-full-access` 才把门锁拆掉。

Codex 的 `sandbox_mode` 有三档，对应笼子把可写范围画多大：
...
关键洞见：**审批和沙箱叠在同一条路上，各管一问，不是「区内审批、区外沙箱」。**

- 审批（s03）问「这一下要不要先问人」——屋里的危险动作（如 `rm -rf`）沙箱往往拦不住，因为路径仍在可写根里。
- 沙箱（本章）问「这一下物理上碰得到什么」——屋里屋外都在笼子里；`sandbox_mode` 只决定可写范围。出界由笼子先挡，不靠你一条条点路径。

真实 Codex 里顺序是：先在当前沙箱里跑；需要更大权限（出界、联网）且 `approval_policy` 允许问人，再弹审批，批准后提权重跑。教学版把两层拆开：本章越界直接 `blocked` 喂回，不接着问 `y/n`。对模型来说，「被拒」只是一条普通 `function_call_output`，循环照常 continue。
...
正如 s03 源码解读所说，Codex 里二者在同一条执行路径上：一条命令先按 `sandbox_mode` 在受限环境里试跑（工作区内的写入也是这个进程、直接落在真实文件上，不是副本同步）；若它需要更高权限（要写工作区外、要联网）且当前 `approval_policy` 允许问人，harness 弹出审批，批准后用提升的权限重跑。教学版把「先试跑、失败再升级」简化成了「沙箱直接拒 + 错误 item 喂回」，方向一致。
