...
审批解决的是「要不要问人」，但即使你点了 `y`，命令仍以全部权限跑在真实工作区上——一个被批准的 `rm -rf /` 照样能毁掉系统。没有沙箱时，连「这个路径在不在工作区」都得靠人一条条点。

s04 Sandbox → 用 `sandbox_mode` 给**每一次**调用套上笼子（工作区内也在笼子里，不是区外才有）：只读 / 只能写工作区 / 完全放开。审批管「问不问」，沙箱管「碰得到什么」。
